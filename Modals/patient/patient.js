...
const billSchema = new mongoose.Schema({
  ...
  billedBy: { type: mongoose.Schema.Types.ObjectId, ref: 'Doctor' },
  fileUrl: String
}, { _id: false });
...
const patientSchema = new mongoose.Schema({
  ...
  doctorId: { type: mongoose.Schema.Types.ObjectId, ref: 'Doctor', required: true }, // important
...
// 🔹 Validate billNo for new/modified bills
patientSchema.pre('save', function(next) {
  if (this.isModified('bills')) {
    this.bills.forEach((bill, idx) => {
      const billModified = bill.isNew || (typeof bill.isModified === 'function' && bill.isModified('billNo'));
      if (billModified && !bill.billNo) {
        return next(new Error(`billNo is required for bill at index ${idx}`));
      }
    });
  }
  next();
